...
export const SoundFontModal: React.FC<SoundFontModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col text-slate-100 animate-in fade-in zoom-in-95 duration-150">
        {/* ヘッダー */}
        ...
        <div className="p-5 space-y-5 overflow-y-auto max-h-[75vh]">
          ...
          <div className="bg-slate-800/80 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-xs text-slate-400 font-medium">現在使用中の音源</div>
                <div className="text-sm font-semibold text-white flex items-center gap-2 mt-0.5">
                  ...
                  {sfState.currentSoundFont?.size && (
                    <span className="text-[11px] px-2 py-0.5 rounded-full bg-indigo-900/60 text-indigo-300 border border-indigo-700">
            ...
            <div>
              {sfState.status === 'ready' ? (
                <span className="flex items-center gap-1.5 text-xs font-medium text-emerald-400 bg-emerald-950/60 border border-emerald-800 px-2.5 py-1 rounded-full">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  準備完了
                </span>
              ) : sfState.status === 'loading' ? (
                <span className="flex items-center gap-1.5 text-xs font-medium text-amber-400 bg-amber-950/60 border border-amber-800 px-2.5 py-1 rounded-full">
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ロード中 ({sfState.progress}%)
                </span>
              ) : (
                <span className="flex items-center gap-1.5 text-xs font-medium text-rose-400 bg-rose-950/60 border border-rose-800 px-2.5 py-1 rounded-full">
          ...
          {errorMessage && (
            <div className="p-3 bg-rose-950/80 border border-rose-700 text-rose-300 text-xs rounded-lg flex items-center gap-2">
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center space-y-2">
              <div className="p-3 bg-slate-800 rounded-full text-indigo-400 shadow-inner">
                {isProcessing ? (
              ...
              <div>
                <span className="text-sm font-medium text-indigo-300 hover:underline">
                  SoundFont ファイルを選択
                </span>
                <span className="text-sm text-slate-400"> またはここにドラッグ＆ドロップ</span>
              </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400 px-1">
              <span className="font-semibold">利用可能な音源ライブラリ</span>
              <span>IndexedDB キャッシュ</span>
            </div>
                  ...
                  >
                    <div className="flex items-center space-x-2.5 overflow-hidden pr-2">
                      ...
                      <div className="truncate">
                        ...
                        <div className="text-[10px] text-slate-400">{formatSize(sf.size)}</div>
                      </div>
                    </div>

                    <div className="flex items-center space-x-1.5 shrink-0">
                      {isCurrent ? (
                        <span className="flex items-center gap-1 text-[11px] font-semibold text-indigo-400 bg-indigo-950 px-2 py-1 rounded border border-indigo-800">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 text-xs font-medium bg-slate-700 hover:bg-slate-600 active:bg-slate-500 text-white rounded transition-colors"
                        >
                          選択
                        </button>
                      )}
          ...
          {/* ヒント情報 */}
          <div className="p-3 bg-slate-800/40 border border-slate-800 rounded-lg flex items-start space-x-2 text-[11px] text-slate-400">
        ...
        <div className="px-5 py-3 border-t border-slate-800 bg-slate-950 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-1.5 text-xs font-semibold bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-white rounded-md transition-colors shadow-sm"
          >
            閉じる
          </button>
        </div>
